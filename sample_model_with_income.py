...
# Designed to run as of end of last month (Custom Entered) given you are running it after the start of the current
# month. The last data row from portfolio ledger is removed to account for as of last month. **ATTENTION**

# ----------------STEPS TO RUN THE FULL MODEL-----------------------
# For Accumulation Only - Use Dynamic BAV assuming No Rider Fee
...
def get_asset_prices_bloomberg(sdate, edate, data_type='prices', return_freq='daily'):
	...
	if data_type == 'prices':
		...
		inputs_df = pd.read_excel(src + 'portfolio_information.xlsx', sheet_name='asset_weights', index_col=[0],
								  parse_dates=True)
		sp_500_forecast = pd.read_excel(src + 'portfolio_information.xlsx', sheet_name='income_model_inputs',
										index_col=[0],
										parse_dates=True)
		sp_estimated_ret = float(sp_500_forecast.loc['sp500_forecast', 'inputs'])
...
def portfolio_analytics(file_path, fia_name, fia_wt):
	...
	# ---Read SPXT Daily Prices for Drawdown Comparision
	df_spy = pd.read_csv(src + 'benchmark_prices.csv', index_col=[0], parse_dates=True)
	df_spy = df_spy['SPXT Index']
	df_spy = df_spy.resample('BM', closed='right').last()
	...
	try:
		...
		monthly_drawdown.to_csv(file_path + fia_name + '_monthly_drawdown.csv')
	...
	reg_df.dropna(inplace=True)
	df_spy = df_spy.loc[reg_df.index[0]: reg_df.index[-1]]
	
	# -------------------------------DRAWDOWN statistics--------------
	df = pd.read_csv(src + 'benchmark_prices.csv', index_col=[0], parse_dates=True)
	df = df['SPXT Index']
	spy_index = df.resample('BM', closed='right').last()
	...
	df_data = pd.DataFrame(index=reg_df.index)
	# df_data['ret'] = reg_df.loc[:, reg_df.columns[1]].add(1)
	# df_data['ret'] = bm_df.pct_change().add(1)
	# df_data['ret'] = spy_index.pct_change().add(1)
	df_data['ret'] = df/df.iloc[0]
	cum_returns = df_data['ret'].add(1).cumprod()
	drawdown = cum_returns.div(cum_returns.cummax()) - 1
	
	df_data['modMax'] = df_data.ret.cummax()
	df_data['modDD'] = 1 - df_data.ret.div(df_data['modMax'])
	groups = df_data.groupby(df_data['modMax'])
	dd = groups['modMax', 'modDD'].apply(lambda g: g[g['modDD'] == g['modDD'].max()])
	top10dd = dd.sort_values('modDD', ascending=False)
	
	dd_col = ('Previous High Date', 'New High Date', 'Peak Value', '% Drawdown', 'Floor Date', 'Peak_to_Floor',
			  'Floor_to_New_Peak', 'Peak_to_Peak')
	df_dd = pd.DataFrame(columns=dd_col)
	
	for i in range(1, len(top10dd)):
		index_list = top10dd[i - 1:i].index.tolist()[0]
		# print(index_list)
		start, end, peak, dd, dd_date, dd_length, dd_rec, tot_length = drawdown_group(df_data, index_list)
		# print(start,end,dd,dd_date,dd_length,dd_rec,tot_length)
		df_dd.loc[i - 1] = drawdown_group(df_data, index_list)
	df_dd = df_dd[['Previous High Date', 'Floor Date', 'New High Date', 'Peak Value', '% Drawdown',
				  'Peak_to_Floor', 'Floor_to_New_Peak', 'Peak_to_Peak']]
	s_date = list(df_dd['Previous High Date'])
	# s_date = s_date[:5]
	e_date = list(df_dd['Floor Date'])
	# e_date = e_date[:5]
	
	for counter in range(len(s_date[:5])):
		per_ret = frame2.loc[e_date[counter]]/frame2.loc[s_date[counter]] - 1
		df_dd.loc[df_dd.index[counter], 'Current'] = per_ret.values[0]
		df_dd.loc[df_dd.index[counter], 'Proposed'] = per_ret.values[1]
		
	select_cols = ['Previous High Date', 'Floor Date', 'New High Date', '% Drawdown', 'Current', 'Proposed']
	df_dd = df_dd[select_cols]
	# Output only the 5 largest drawdown
	df_dd = df_dd.head()
	df_dd.loc[:, ['% Drawdown', 'Current', 'Proposed']] = df_dd.loc[:, ['% Drawdown', 'Current', 'Proposed']].applymap(
		lambda x: abs(x))
	df_dd.to_csv(src + 'drawdown_slide.csv')
	# --------------------DRAWDOWN Statistics ENDS-------------------------------
...
def format_marketing_slides(file_path, fia_cols):
	...
	page_3_c.loc[page_3_c.index[0], 'Footnotes'] = ('This is the example footnote. The Nationwide New Heights® {} FIA '
	...
	page_6b.reset_index(inplace=True)
	text = (
		"Returns for the Current Portfolio are historical. Returns for the Proposed Portfolio are historical for the" \
		...
		"be guaranteed").format(product_life_in_years, str(perc_advisor_fee) + '%')
	page_6b.loc[:, 'Dummy'] = ''
	...
	text2 = ("For every dollar risked, the Proposed Portfolio returns an additional {}. The Proposed Portfolio’s"
			 " Sharpe Ratio is {} {} the Current Portfolio and {} {} the S&P 500® Total Return Index.")\
		.format(perc_x1, perc_x2, rel_to_curr, perc_x3, rel_to_sp)
	...
	text2 = ("For every dollar risked, the Proposed Portfolio returns an additional {}. The Proposed Portfolio’s " \
			 "Sharpe Ratio is {} higher than the Current Portfolio and {} higher than the S&P 500® Total Return " \
			 "Index. ").format(x1, x2, x3)
	...
	if 'moz' in product_name:
		...
		text3 = 'The NYSE® Zebra Edge® Index has been licensed to Nationwide Life and Annuity Insurance Company' \
				' (“Nationwide”). Neither Nationwide nor the Nationwide New Heights® Fixed Indexed Annuity' \
				' (the "Product") is sponsored, operated, endorsed, recommended, sold or promoted by Zebra,' \
				' NYSE or UBS and in no event shall Zebra, NYSE or UBS have any liability with respect to the' \
				' Product or the Index. Zebra, NYSE and UBS make no representations, give no express or implied' \
				' warranties and have no obligations with regard to the Index, the Product, the client or other third' \
				' party. The marks NYSE®, and NYSE Arca® are registered trademarks of NYSE Group, Inc., ' \
				'Intercontinental Exchange, Inc. or their affiliates. The marks Zebra® and Zebra Edge® are trademarks' \
				' of Zebra, may not be used without prior authorization from Zebra, and are being utilized by NYSE' \
		...
		text4 = 'Dynamic Participation Rates: Fixed indexed annuities were first established in 2000, and the FIAs' \
				' comprising the NYSE® Zebra Edge® Index were not available for the entire time period' \
...
def income_model_constant_portfolio_returnv3(base, starting_income=0.0, num_of_years=30, method='normal'):
	...
	# ----------------CONSTANT FIA INDEX GROWTH RATE. Growth Assumes NET OF RIDER FEES etc.-------------------
	income_df.loc[:, 'term_ret_netspr'] = const_fia_index_ret
	counter_end = income_starts
	
	# Not needed if the custom start date is used where start date is dated a year back manually to capture full returns
	income_starts = income_starts + 2
...
if __name__ == "__main__":
	...
	# feature_selection_model(start, end, "monthly")
	# ---------------------------Functional Methods BEGINS:--------------------------------
	...
	# Version 1 - Original for base portfolio - NO RIDER FEE Assumed
	...
	# Version 3 - for base portfolio with the RIDER FEES INCLUDED with dynamic BAV
	# portfolio_construction_version3(blended_fia_ts, fia_cols, product_life_in_years, True, perc_advisor_fee,
	# 								fee_frequency)
	
	# Version 3 - for FIA portfolio with the rider fee included with Dynamic BAV
	# portfolio_construction_version3(blended_fia_ts, fia_cols, product_life_in_years, False, perc_advisor_fee,
	# 								fee_frequency)
	
	# Version 3 - for base portfolio with the RIDER FEES INCLUDED with Static BAV
	# portfolio_construction_static_version3(blended_fia_ts, fia_cols, product_life_in_years, True, perc_advisor_fee,
	# 									   fee_frequency)
	
	# Version 3 - for FIA portfolio with the RIDER FEES INCLUDED with Static BAV
	...
	# RIDER FEE is included in the accumulation only model (portfolio_construction_version3). The income mode assumes
	# 0 RIDER FEES to avoiud double counting between the accumulation model and income model. The Income model uses the
	# portfolio returns from the Accumulation model which includes rider fees.
	...
	# consistent for both. Independent of Accumulation as portfolio and FIA grows as constant returns. Constant Return
	# is NET OF RIDER FEES ETC.
